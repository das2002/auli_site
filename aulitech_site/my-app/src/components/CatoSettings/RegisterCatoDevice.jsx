...
import { doc, setDoc, where, collection, addDoc, deleteDoc, query, updateDoc, deleteField } from "firebase/firestore"; 
...
const RegisterCatoDevice = ({user}) => {
  ...
  const addDeviceDoc = (jsonData) => {
    ...
    try {
      const storeFirst = async() => {
        try {
          const colRef = collection(db, "users");
          // console.log(colRef);

          const delQuery = query(doc(db, 'users', user.uid, 'userCatos', deleteId))
          await deleteDoc(delQuery);
          console.log("deleted");

          // const docRef = doc(db, 'users', user.uid, 'userCatos', deleteId);
          // await updateDoc(docRef, {
          //   initialize: deleteField(),
          ...
          console.log("store first device error: ", error);
        }
      }
  
      const storeAnother = async() => {
        try {
          const colRef = collection(db, "users");
          await addDoc(collection(colRef, user.uid, 'userCatos'), {
            hardwareuid: hwUid,
