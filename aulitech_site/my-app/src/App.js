...
import StoreRegisterData from "./components/CloudFirestore/StoreRegisterData";
...
import { doc, setDoc, addDoc, collection, query, getDocs, getDoc, where } from "firebase/firestore";
...
function App() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      let userAlreadyThere = await checkUserExists(userJSON.uid);
      console.log(userAlreadyThere)
      
      // if the user does not exist in the users database right now
      if (!userAlreadyThere) {
        ...
        await addDoc(collection(colRef, userJSON.uid, 'userCatos'), {
